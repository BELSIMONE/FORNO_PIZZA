...
void loop() {
    ...
    if (temperature1 <= tempimpostata1 - deltaT1 && (temperature2 + 50) >= (tempimpostata2 - deltaT2)) {
        digitalWrite(rele1, HIGH); // Accende il relè 1 solo se la condizione aggiuntiva è soddisfatta
    } else  {
        digitalWrite(rele1, LOW); // Spegne il relè 1
    }

    // Controllo e attivazione del relè 2 con isteresi e gestione del tempo senza delay


    if (temperature2 <= tempimpostata2 - deltaT2) {
        if (digitalRead(rele1) == HIGH) {
            if (!rele2Status &&  (currentMillis - rele2LastChangeTime < 2000)) {
                digitalWrite(rele2, HIGH);          // Accende il relè 2
                rele2LastChangeTime = currentMillis; // Salva l'ora corrente
                rele2Status = true;                 // Stato di relè 2 attivo
            } else if (currentMillis - rele2LastChangeTime >= 2000) {
                digitalWrite(rele2, LOW);           // Spegne il relè 2 dopo 2 secondi
                rele2LastChangeTime = currentMillis; // Resetta il timer per i 2 secondi di spegnimento
                rele2Status = false;                // Stato di relè 2 inattivo
            }
        } else {
            digitalWrite(rele2, HIGH);  // Accende il relè 2 se il relè 1 non è acceso
            rele2Status = true;         // Aggiorna lo stato del relè 2
        }
    } else if (temperature2 > tempimpostata2 + deltaT2) {
        digitalWrite(rele2, LOW); // Spegne il relè 2
        rele2Status = false;      // Resetta lo stato del relè 2
    }
    ...
    if (countingDown) {
        if (currentMillis - previousMillis >= interval) {
            ...
            if (countdown > 0) {
                ...
                // Suona una melodia alla fine del conto alla rovescia
