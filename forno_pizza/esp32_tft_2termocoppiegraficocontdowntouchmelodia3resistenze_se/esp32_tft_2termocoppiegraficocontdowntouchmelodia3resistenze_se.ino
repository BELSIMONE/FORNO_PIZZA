...
const int rele1 = 22; // cielo centrale
const int rele2 = 17; // platea
const int rele3 = 13; // Nuovo rele resistenza cielo esterno

unsigned long startupDelay = 20 * 60 * 1000; // 20 minuti in millisecondi
...
void loop() {
 ...
 ArduinoOTA.handle(); 
  ...
  if (currentMillis - systemStartTime < startupDelay) {
    // Mantieni i rele 1 e 3 spenti durante il ritardo di avvio
    digitalWrite(rele1, LOW);
    digitalWrite(rele3, LOW);
  } else {
    // Qui va il codice per controllare e attivare i rele 1 e 3 normalmente
    // Controllo e attivazione dei rele 1 e 3 con isteresi
    if (temperature1 < tempimpostata1 - deltaT1) {
      digitalWrite(rele1, HIGH); // Accende il rele 1
    } else if (temperature1 > tempimpostata1 + deltaT1) {
      digitalWrite(rele1, LOW); // Spegne il rele 1
    }

    if (temperature2 < tempimpostata2 - deltaT2) {
      digitalWrite(rele3, HIGH); // Accende il rele 3
    } else if (temperature2 > tempimpostata2 + deltaT2) {
      digitalWrite(rele3, LOW); // Spegne il rele 3
    }

    // Controllo e attivazione del relè 2 con isteresi solo se il sistema non è appena avviato
    if (currentMillis - systemStartTime >= startupDelay) {
      if (temperature2 < tempimpostata2 - deltaT2) {
 ...
 // Controllo dei rele
    if (countingDown) {
        // Se il conto alla rovescia è in corso, rele3 ha priorità su rele2
        if (digitalRead(rele1) == HIGH) {
            digitalWrite(rele3, HIGH); // Accende rele3 se rele1 è acceso
            digitalWrite(rele2, LOW);  // Spegne rele2
        } else {
            digitalWrite(rele3, LOW); // Altrimenti spegne rele3
        }
    } else {
        // Se il conto alla rovescia non è attivo, applica la logica precedente
        if (digitalRead(rele1) == HIGH && digitalRead(rele2) == LOW) {
            digitalWrite(rele3, HIGH); // Accende rele3 se rele1 è acceso e rele2 è spento
        } else {
            digitalWrite(rele3, LOW); // Altrimenti spegne rele3
        }
    }
