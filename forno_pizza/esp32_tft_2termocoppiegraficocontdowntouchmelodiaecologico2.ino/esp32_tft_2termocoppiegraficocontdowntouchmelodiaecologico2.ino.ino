...
void loop() {
    ...
    if (temperature1 <= tempimpostata1 - deltaT1  ) {
        if ((temperature2 + 50) <= (tempimpostata2 - deltaT2)) {
         rele2Priority = true;
         if (currentMillis - rele1LastChangeTime >= 2000) {
    ...
    if (!rele2Priority) {
    //if (digitalRead(rele1) == HIGH) { // Solo se il relè 1 è acceso
        if (currentMillis - rele2LastChangeTime >= 2000) {
